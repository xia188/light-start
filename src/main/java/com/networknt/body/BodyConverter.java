    ...
    static Map<String, Object> convert(FormData data) {
        ...
        for (String key : data) {
            if (data.get(key).size() == 1) {
            	FormValue formValue = data.getFirst(key);
                Object value = formValue.isFileItem() ? formValue.getFileItem() : formValue.getValue();
                map.put(key, value);
            } else if (data.get(key).size() > 1) {
                List<Object> list = new ArrayList<>();
                for (FormData.FormValue value : data.get(key)) {
                    list.add(value.isFileItem() ? value.getFileItem() : value.getValue());
                }
                map.put(key, list);
            }
            // ignore size == 0
